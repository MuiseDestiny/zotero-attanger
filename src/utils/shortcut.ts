...
export function registerShortcut(prefKey: string, callback: Function) {
  const shortcutArr = (Zotero.Prefs.get(`${config.addonRef}.${prefKey}`) as string).split(" + ")
  const allModifiers = ["Shift", "Meta", "Ctrl", "Alt", "Control"]
  const modifiers = shortcutArr.filter(i => allModifiers.includes(i)).map(i => i.toLowerCase())
